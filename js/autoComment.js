...
function getLevelFromRadio(groupName) {
...
function extractCoreIdea(text) {
  const cleaned = text
    .replace(/^Học sinh\s+/i, "")
    .replace(/^Trong.*?,\s*/i, "")
    .replace(/^Bước đầu\s+/i, "")
    .replace(/^Đôi khi\s+/i, "")
    .replace(/^Cần\s+/i, "")
    .replace(/\.$/, "");
...
function collectSessionData(subjectKey, tabId) {
  const config = SUBJECT_CONFIG?.[subjectKey];
  if (!config) return [];

  const blocks = config.sessions?.[tabId];
  if (!blocks) return [];

  const result = [];

  blocks.forEach((block, blockIndex) => {
    block.criteria.forEach((_, cIndex) => {
      const cKey = `block${blockIndex + 1}_tc${cIndex + 1}`;
      const groupName = `${tabId}_${cKey}`;
      const level = getLevelFromRadio(groupName);

      if (!level) return;

      const text =
        AUTO_COMMENT_MAP?.[subjectKey]?.[tabId]?.[cKey]?.[level];

      if (!text) return;

      result.push({
        level,
        idea: extractCoreIdea(text)
      });
    });
  });
...
function mergeIdeas(ideas) {
  const shortIdeas = ideas.map(i => i.idea);
  const unique = [...new Set(shortIdeas)];

  return unique.slice(0, 3); 
}

function groupBySkillWithPriority(items) {
  const skillMap = {};
  ...
  Object.keys(skillMap).forEach(skill => {
    const levels = skillMap[skill];

    const hasM1 = levels.some(l => l.level === "M1");

    if (hasM1) {
      const m1Item = levels.find(l => l.level === "M1");
      result.push({ ...m1Item, finalLevel: "M1" });
    } else {
      const highest = levels.find(l => l.level === "M3") ||
                      levels.find(l => l.level === "M2");

      if (highest) {
        result.push({ ...highest, finalLevel: highest.level });
      }
    }
  });
...
// ================= TẠO NHẬN XÉT CHUNG =================
function buildGeneralComment(items) {
  if (!items.length) return "";

  const { subjectStart } = getStudentPronouns();

  const filtered = items.filter(i => i.level !== "M3");
  const baseItems = filtered.length ? filtered : items;

  const ideas = mergeIdeas(baseItems).slice(0, 2); 

  const introOptions = [
    `Trong buổi học, ${subjectStart}`,
    `Qua hoạt động thực hành, ${subjectStart}`,
    `Trong quá trình làm việc, ${subjectStart}`
  ];

  const intro = introOptions[Math.floor(Math.random() * introOptions.length)];

  if (ideas.length === 1) {
    return `${intro} ${ideas[0]}.`;
  }

  return `${intro} ${ideas[0]}. ${ideas[1].charAt(0).toUpperCase() + ideas[1].slice(1)}.`;
}

// ================= ĐIỂM NỔI BẬT =================
function buildHighlightComment(items) {

  const { subjectStart } = getStudentPronouns();

  const m3 = items.filter(i => i.level === "M3");

  if (!m3.length) {
    return {
      text: `${subjectStart} thể hiện tinh thần học tập tích cực và nền tảng kỹ năng ổn định.`,
      usedSkills: []
    };
  }

  const selected = m3.slice(0, 2);

  return {
    text: `${subjectStart} nổi bật ở ${selected.map(i => i.idea).join(" và ")}.`,
    usedSkills: selected.map(i => i.skill)
  };
}

// ================= HƯỚNG PHÁT TRIỂN =================
function buildDevelopmentComment(items) {

  const { subjectMid } = getStudentPronouns();

  const m1 = items.filter(i => i.level === "M1");
  const m2 = items.filter(i => i.level === "M2");

  // Ưu tiên M1
  if (m1.length) {
    const selected = m1.slice(0, 2);

    return `Để tiến bộ hơn, ${subjectMid} ${selected
      .map(i => normalizeDevelopmentText(i.idea))
      .join(" và ")}.`;
  }

  // Nếu không có M1 → dựa vào M2 để nâng lên
  if (m2.length) {
    const selected = m2.slice(0, 1);

    return `Để phát triển thêm, ${subjectMid} có thể rèn luyện sâu hơn ở phần ${selected[0].idea}.`;
  }

  // Nếu toàn M3 → mở rộng theo chiều sâu
  return `Để phát triển thêm, ${subjectMid} có thể thử áp dụng kiến thức vào những yêu cầu phức tạp hơn hoặc chủ động mở rộng ý tưởng của mình.`;
}

// ================= RENDER =================
function renderAutoCommentForSession(tabId) {
  const subjectKey = document.getElementById("monTraiNghiem")?.value;
  if (!subjectKey) return;

  const items = collectSessionData(subjectKey, tabId);

  const general = buildGeneralComment(items);

  const highlightResult = buildHighlightComment(items);
  const highlight = highlightResult.text;

  const dev = buildDevelopmentComment(items);

  document.getElementById(`commentGeneral_${tabId}`).value = general;
  document.getElementById(`commentHighlight_${tabId}`).value = highlight;
  document.getElementById(`commentDev_${tabId}`).value = dev;
}
...
function bindAutoCommentEvents() {
  document.addEventListener("change", function (e) {
    ...
    const tabId = target.name.split("_")[0];
    renderAutoCommentForSession(tabId);
  });
}

window.bindAutoCommentEvents = bindAutoCommentEvents;
window.renderAutoCommentForSession = renderAutoCommentForSession;

window.__SESSION_STORE__ = window.__SESSION_STORE__ || {};
...
function renderAutoCommentForSession(tabId) {
  ...
  window.__SESSION_STORE__ = window.__SESSION_STORE__ || {};
  window.__SESSION_STORE__[tabId] = items;
...
// =========== XỬ LÝ CÂU TIÊU CỰC ============
function normalizeDevelopmentText(text) {
  return text
    .replace("chưa hình thành thói quen", "rèn thêm thói quen")
    .replace("cần nhắc để", "")
    .replace("cần", "")
    .replace("đôi khi", "")
    .replace(/\.$/, "")
...
function refineDevelopmentIdea(text) {

  if (text.includes("làm việc rời rạc")) {
    return "phát triển thêm kỹ năng làm việc nhóm và phối hợp chặt chẽ hơn";
  }

  if (text.includes("chưa hình thành thói quen chạy thử")) {
    return "rèn thói quen chạy thử thường xuyên";
  }

  if (text.includes("quan sát kết quả")) {
    return "chủ động quan sát kết quả sau mỗi lần thay đổi";
  }

  if (text.includes("phụ thuộc")) {
    return "tăng tính tự chủ khi thực hiện nhiệm vụ";
  }
...
function buildFinalSummary() {
  ...
  const { subjectStart, subjectMid } = getStudentPronouns();

  const allItems = Object.values(window.__SESSION_STORE__ || {}).flat();
  if (!allItems.length) return;

  const m3 = allItems.filter(i => i.level === "M3");
  const m2 = allItems.filter(i => i.level === "M2");
  const m1 = allItems.filter(i => i.level === "M1");

  const m3Ideas = mergeIdeas(m3);
  const m1Ideas = mergeIdeas(m1);

  // ===== NHẬN XÉT CHUNG =====
  let generalText = "";

  if (m3.length >= m1.length && m3.length > 0) {

    generalText =
      `${subjectStart} cho thấy sự tiến bộ rõ rệt sau các buổi học. ` +
      `Từ việc tiếp cận nội dung ban đầu, ${subjectMid} dần chủ động hơn trong cách triển khai ý tưởng, ` +
      `biết điều chỉnh chương trình khi gặp vấn đề và hoàn thiện sản phẩm một cách mạch lạc hơn. ` +
      `Khả năng xử lý yêu cầu và duy trì tiến độ làm việc ngày càng ổn định.`;

  } else if (m1.length > 0) {

    generalText =
      `${subjectStart} đã nắm được các nội dung nền tảng và từng bước hoàn thành yêu cầu trong mỗi buổi học. ` +
      `Tuy nhiên, một số kỹ năng như kiểm soát logic, tính chính xác khi lập trình hoặc khả năng tự xử lý lỗi ` +
      `chưa thật sự ổn định, khiến quá trình thực hiện đôi lúc còn chậm hoặc thiếu sự chắc chắn.`;

  } else {

    generalText =
      `${subjectStart} duy trì được sự tập trung và hoàn thành đầy đủ các nhiệm vụ theo tiến độ học. ` +
      `Kỹ năng thực hành được củng cố dần qua từng buổi và thể hiện sự ổn định trong quá trình làm việc.`;
  }
  ...
  if (m3Ideas.length) {
    highlightText =
      `${subjectStart} nổi bật ở ${m3Ideas.slice(0, 2).join(" và ")}.`;
  } else {
    highlightText =
      `${subjectStart} có tốc độ tiếp thu khá nhanh và thích nghi tốt với nội dung mới.`;
  }
  ...
  const selected = m1Ideas
      .slice(0, 2)
      .map(refineDevelopmentIdea);

  devText =
      `Trong tương lai, ${subjectMid} có thể ${selected[0]}.`;

  if (selected[1]) {
      devText += ` Đồng thời, ${subjectMid} nên ${selected[1]}.`;
  }

  } else {

  devText =
      `${subjectMid} có thể thử nâng độ phức tạp của sản phẩm để mở rộng kỹ năng.`;
  }

  document.getElementById("commentGeneral_summary").value = generalText;
  document.getElementById("commentHighlight_summary").value = highlightText;
  document.getElementById("commentDev_summary").value = devText;
}